...
export default function DebateModal({ signal, activePersonas, preference, onClose }) {
  ...
  const messages = signal.debate.filter((m) => activePersonas[m.persona]);
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="max-w-3xl w-full bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col shadow-2xl" style={{ maxHeight: '90vh' }}>
        <div className="flex items-start justify-between p-6 border-b border-zinc-800">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <MessageSquare className="w-4 h-4" style={{ color: YELLOW }} />
              <span className="text-xs font-bold uppercase tracking-wider" style={{ color: YELLOW }}>Persona Debate</span>
            </div>
            <h2 className="text-lg font-semibold text-white leading-snug pr-4">{signal.title}</h2>
          </div>
          <button onClick={onClose} className="p-1 rounded-lg text-zinc-500 hover:text-white hover:bg-zinc-800 transition">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {visibleCount >= messages.length && messages.length > 0 && (
          <div className="p-6 border-t border-zinc-800">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs uppercase tracking-wider text-zinc-500 mb-1">Synthesized Recommendation</div>
              ...
              <div className="flex gap-1.5">
                {Object.entries(signal.personaVotes).filter((entry) => activePersonas[entry[0]]).map((entry) => {
                  const pid = entry[0];
                  const vote = entry[1];
                  ...
                  return (
                    <div key={pid} className="text-center">
                      <div
                        ...
                        style={{ backgroundColor: vrec.bg, color: vrec.text, fontSize: 10 }}
                      >
                        {p.name[0]}
                      </div>
                      <div className="text-zinc-500 mt-1" style={{ fontSize: 9 }}>{vote}</div>
                    </div>
                  );
                })}
              </div>
