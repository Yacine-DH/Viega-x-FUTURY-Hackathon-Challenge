...
graph TD
    subgraph Acquisition [Data Acquisition - External Web]
        A[Python Scrapers / APIs] -->|POST Raw JSON| B[n8n Webhook Receiver]
    end

    subgraph GCP [Google Cloud Platform - Backend]
        subgraph Integration [n8n Integration Layer]
            B --> C{LLM Noise Filter Node}
            C -->|Irrelevant| D[Discard]
            C -->|Verified Signal| E[n8n Data Formatter]
        end

        subgraph Reasoning [Vertex AI Reasoning Engine]
            E --> F[LLM: Score & Assign Confidence]
            F --> G[LLM: Generate Decisions & Arguments]
            G --> H[(Firestore Database)]
        end
        
        subgraph HITL [Human-in-the-Loop Engine]
            N[Receive User Feedback] --> O{Vertex AI: Persona Tribunal}
